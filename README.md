...
- **Locations** — open-meteo geocoding search, a two-step _Use my location_ (a permission-free approximate fix from the Cloudflare edge first, precise GPS on a second tap — see [Locate flow](#locate-flow)), URL-shareable state, favourites and recent-search in localStorage.
...
- **wrangler** for deploys to Cloudflare Workers (static assets plus the `worker/` package)
...
pnpm dev:worker      # wrangler dev on :8080 — the real Worker serving dist/ (build first)
pnpm test:worker     # the Worker's own vitest suite (runs in workerd)

pnpm deploy          # build + wrangler deploy from worker/ (Cloudflare Workers)
pnpm deploy:preview  # build + upload a preview version aliased to the current commit
```

`pnpm dev` and `pnpm preview` proxy `/api` to production, so the locate flow works without a local Worker. To run against a local one instead:

```bash
pnpm dev:worker                                        # wrangler dev on :8080
METEOCOMPARE_API_PROXY=http://localhost:8080 pnpm dev  # frontend proxies /api → local worker
```

The lint script is the single quality gate — it runs the linter, asserts formatting, and type-checks in one command, for the SPA (`lint:web`) and the Worker package (`lint:worker`). A single `pnpm install` at the repository root installs both.

## Deploy

The app is shipped as a Cloudflare Worker with static assets: the Vite build in `dist/` plus the small Worker in `worker/` that answers `/api/geo` (see [Locate flow](#locate-flow)). The Worker is its own pnpm workspace package (`meteocompare-worker`) with its own `wrangler.jsonc`, tsconfig, lint config and vitest setup, so everything Cloudflare-specific lives in one folder:

- `worker/wrangler.jsonc`
  - `main: src/index.ts` — the Worker entry
  - `assets.directory: ../dist` — the Vite build output
  - `assets.not_found_handling: "single-page-application"` — Cloudflare serves `index.html` for any unmatched non-API path, which is exactly what `vue-router`'s history mode needs
  - `assets.run_worker_first: ["/api/*"]` — only API paths invoke the Worker; every other request is served straight from the asset store, with no Worker invocation and no cold start
- `worker/src/worker-configuration.d.ts` — runtime types generated by `pnpm --filter meteocompare-worker generate-types`; not tracked, the Worker's `lint`, `test` and `type-check` scripts regenerate it first
- `worker/test/` — the Worker's tests, run inside workerd via `@cloudflare/vitest-plugin`

### Locate flow

Browser geolocation raises a permission prompt, and an installed iOS home-screen web app raises it on **every launch** — iOS does not persist the grant for standalone web apps. So _Use my location_ is two-step:

1. The first tap fetches `/api/geo`, a Worker endpoint (`worker/src/geo.ts`) that echoes Cloudflare's IP-derived estimate of the visitor's position — `request.cf` city, region, country, timezone and coordinates. No permission prompt; city-level accuracy on a fixed line, possibly hundreds of kilometres off on a mobile carrier or VPN. The banner flags the result as approximate (`Munich, Bavaria, DE (approx.)`).
2. While that approximate fix is the current location, the same button becomes _Use precise location (GPS)_: a second tap runs the browser Geolocation API, and only then does the prompt appear.

The coordinates come straight from `request.cf`; the city name is only used as a label. A name-based geocoder lookup would add a round trip and a wrong-namesake failure mode without improving on the IP estimate it starts from.

The client (`src/api/edgeGeo.ts`) treats anything but a JSON 200 as "no estimate": the Worker's 204 for an unplaceable IP, a plain static host answering with the SPA shell, a network error, or a reply slower than 2.5 s. In every such case the first tap falls straight through to GPS, so the button never dead-ends. `vite dev` and `vite preview` proxy `/api` to production (or to a local Worker via `METEOCOMPARE_API_PROXY`), so Vite's SPA fallback never answers an API path itself.

Privacy: the response is derived from the visitor's IP and is served `cache-control: no-store`, so neither the browser nor a shared cache can hand one visitor's city to the next. Nothing is logged by the endpoint itself.
...
Pushes to `main` deploy to production automatically via the `deploy` job in CI, which runs wrangler from `worker/`.
