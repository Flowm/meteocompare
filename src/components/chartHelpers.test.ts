...
import { buildDayRanges, isVarActive, nextCombinableView } from "./chartHelpers";
...
describe("day ranges from UTC archive solar data", () => {
  ...
  it.each(cases)("pairs each sunrise with the same day's sunset in $name", ({ offset, rise, set, expected }) => {
    const response = {
    ...
    expect(buildDayRanges(times, solar.sunrise, solar.sunset)).toEqual(expected);
  });
  it("clips days at both visible window boundaries and drops days outside it", () => {
    const times = Array.from({ length: 24 }, (_, i) => shiftIsoTime("2026-06-15T12:00", i * 3600));
    expect(buildDayRanges(times, ["2026-06-14T19:24", "2026-06-15T19:24", "2026-06-16T19:24"], ["2026-06-15T09:58", "2026-06-16T09:58", "2026-06-17T09:58"])).toEqual([[7, 22]]);
  });
  it("covers the whole window when it opens and closes in daylight", () => {
    ...
    expect(buildDayRanges(times, ["2026-06-15T05:00", "2026-06-16T05:00"], ["2026-06-15T21:00", "2026-06-16T21:00"])).toEqual([[0, 11]]);
  });
