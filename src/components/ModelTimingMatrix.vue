...
<template>
  <div v-if="hasRows" class="border-ink-700 bg-ink-900/40 overflow-x-auto border p-3">
    ...
    <div class="min-w-[44rem]">
      ...
      <div class="text-paper-500 mb-1 flex items-center font-mono text-[10px] tracking-wide">
        <div class="bg-ink-900 sticky left-0 z-10 w-24 shrink-0 pr-2">lead</div>
        <div class="flex flex-1">
          <div v-for="d in dayCount" :key="d" class="flex-1 text-center tabular-nums">d{{ d - 1 }}</div>
        </div>
      </div>
    ...
    <!-- Legend mirrors HitMissStrip's tones. -->
