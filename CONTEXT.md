...
**Per-model scorecard**:
The table scoring each **Model** (and the **Aggregate**, ranked inline) over the full run window, sorted by composite score. The per-model lens, as opposed to the aggregate-per-day daily breakdown. Carries no predictability — predictability is defined only over the aggregate (see "Predictability"). When the location has stored tuned weights (training page), a second **Aggregate (tuned)** row is scored inline and the heuristic row is relabelled **Aggregate (default)**, for a direct comparison. Bare "Aggregate" elsewhere (the chart, the forecast page) means whichever weighting is currently active — so both rows are qualified here to avoid overloading the term.
